   ...
   do for x in `seq 5`;
      do echo Iteration $m-$w:$x;
         ...
         do cp rlsnake_"$d"_"$m".mod rlsnake_"$d"_"$w"x"$w"_"$x"_"$m".mod;
            ./rlsnake_train -i 1 -s 100 -r 5000 -c rlsnake_"$d"_"$m".mod "$d" -w $w -h $w -rnd 0.02 -gdl 0.01;
         done;  
         cp rlsnake_n8_$m.mod rlsnake_n8_"$w"x"$w"_"$x"_$m.mod;
         ./rlsnake_train -w $w -h $w -i 1 -s 100 -r 5000 -c rlsnake_n8_$m.mod -8 -rnd 0.02 -gdl 0.01;
      done;
  done;
done
      ...
      do for x in `seq 5`;
         do echo Iteration "$m1"vs"$m2"-$w:$x;
            ...
            do cp rlsnake_"$d"_"$m1".mod rlsnake_"$d"_"$w"x"$w"_"$x"_"$m1"_1.mod;
               ...
               ./rlsnake_train -i 1 -s 100 -r 5000 -c rlsnake_"$d"_"$m1".mod "$d" -c rlsnake_"$d"_"$m2".mod "$d" -w $w -h $w -rnd 0.02 -gdl 0.01;
            done;  
            cp rlsnake_n8_$m1.mod rlsnake_n8_"$w"x"$w"_"$x"_"$m1"_1.mod;
            cp rlsnake_n8_$m2.mod rlsnake_n8_"$w"x"$w"_"$x"_"$m2"_2.mod;
            ./rlsnake_train -w $w -h $w -i 1 -s 100 -r 5000 -c rlsnake_n8_$m1.mod -8 -c rlsnake_n8_$m2.mod -8 -rnd 0.02 -gdl 0.01;
         done;
      done;
  done;
done
