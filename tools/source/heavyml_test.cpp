...
{ try
  { if (argc==2 && string("--help")==argv[1] || argc==1)
      throw string("Syntax is usemodel [--model|-m <modelpath>] [--network|-n <network=\"100->[3*[3*[all]],[[all->yes],[all->no]]]\">] [--filename_truths|-fnt <int=0>] <datapath>");
    ...
    { cout << "Label: " << labels[label] << endl;
      ...
      { ifstream fin(datapath+"/"+labels[label]+"/"+*vfile);
        vector<double> data=readvec(fin);
        ...
        // Eval
        vector<double> result=model->Eval(data);
