...
{ try
  { if (argc==2 && string("--help")==argv[1] || argc==1)
    ...
    size_t memory_features=100;
    Network *model=NULL;

    for (size_t arg=1; arg+1<argc; ++arg)
    { if (string("--memory_features")==argv[arg] || string("-mf")==argv[arg])
      { ++arg;
        if (arg+1>=argc)
          throw string("--memory_features must be succeeded by a value");
        stringstream ss;
        ss << argv[arg];
        ss >> memory_features;
        if (memory_features==0)
          throw string("--memory_features must be succeeded by a positive integer");
      }
      else if (string("--gd_repetitions")==argv[arg] || string("-gdr")==argv[arg])
      { ++arg;
        if (arg+1>=argc)
